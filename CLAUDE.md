# Claude Code Project Instructions — quote-kotlin

## Project Overview

This is **quote-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`quote`](https://crates.io/crates/quote).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/quote/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/quote>
